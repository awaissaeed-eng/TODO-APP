...
function renderTasks() {
  ...
  visibleTasks.forEach((task) => {
    ...
    const cat =
      CONFIG.categories && task.category && task.category !== "none"
        ? categoryOf(task.category)
        : null;
...
if (CONFIG.dragReorder) {
  ...
  taskList.addEventListener("drop", () => {
    const newOrderIds = [...taskList.querySelectorAll(".task-item")].map(
      (el) => el.dataset.id,
    );
    const tasks = getTasks();
